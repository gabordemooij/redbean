...
class Dispense extends Base
{
	...
	/**
	 * Test external, custom dispenser. Allows you to use your own
	 * bean objects. This is handy if you need additional performance gains.
	 * By using your own beans you can hard-code some frequently used
	 * methods or properties into those beans.
	 *
	 * @return void
	 */
	public function testUsingCustomDispenser()
	{
		R::getRedBean()->getCurrentRepository()->setExternalDispenser( function( $type ){
			if ( $type === 'custom' ) {
		...
		$bean = R::dispense( 'custom' );
		asrt( ( $bean instanceof \CustomBean ), TRUE );
		$bean->name = 'abc';
		$hash = $bean->quickHash();
		$expected = sha1( $bean );
		asrt( $hash, $expected );
		$bean = R::dispense( 'normal' );
		asrt( ( $bean instanceof \RedBeanPHP\OODBBean ), TRUE );
		asrt( $bean->quickHash(), NULL );
		R::freeze( TRUE );
		R::getRedBean()->getCurrentRepository()->setExternalDispenser( function( $type ){
			if ( $type === 'custom' ) {
				return new \CustomBean;
		...
		asrt( $bean->quickHash(), NULL );
		R::freeze( FALSE );
		R::getRedBean()->getCurrentRepository()->setExternalDispenser( NULL );
		$bean = R::dispense( 'custom' );
		asrt( ( $bean instanceof \RedBeanPHP\OODBBean ), TRUE );
		asrt( $bean->quickHash(), NULL );
		R::freeze( TRUE );
