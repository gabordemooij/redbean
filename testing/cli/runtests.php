...
foreach ($packList as $testPack) {
    ...
    if (file_exists($path.$testPack.'.php')) {
        require $path.$testPack.'.php';
    } elseif (file_exists($hookPath.$testPack.'.php')) {
        require $hookPath.$testPack.'.php';
    }
...
foreach ($report as $file => $lines) {
    ...
    if ($covFilter !== null) {
        if (strpos($file, $covFilter) === false) {
            continue;
        }
    } else {
        if (strpos($file, '/rb.php') === false) {
            ...
            if (strpos($file, 'RedBeanPHP') === false) {
                continue;
            }
        }
    }
